...
const getCommits = async (packageName) => {
  ...
  // i wanted to use '--grep', `"(${packageName})"` here, but there's something up with execa
  ...
  const rePlugin = new RegExp(`^[\\w\\!]+\\(${packageName}\\)`, 'i');
...
const updateChangelog = (commits, cwd, packageName, version) => {
  ...
  const title = `# @rakutentech/${packageName} ChangeLog`;
